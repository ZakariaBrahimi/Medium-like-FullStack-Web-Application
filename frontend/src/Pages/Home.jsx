...
const Home = () => {
  ...
  useEffect(()=>{
    axiosAPI({
      ...
      let home_page_articles = response.data
      setHomePageArticles(home_page_articles)
    }).catch((error)=>{
      console.log(error)
    })
  }, [])
  return (
    <>
      ...
      {homePageArticles.map((article) => (<Link to={`/article/${article['id']}/${article['slug']}`} />))}
      {
        homePageArticles.map((article)=>{
          return (
            <>

    <div key={article['id']} class="p-6 flex flex-col justify-start border-b ">
